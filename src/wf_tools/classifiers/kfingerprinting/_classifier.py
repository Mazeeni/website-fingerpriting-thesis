...
class KFingerprintingClassifier(BaseEstimator, ClassifierMixin):
    ...
    # Mazen: added argument in init to easily switch between NN and non-NN random forest approaches.
    def __init__(self, forest: Optional[RandomForestClassifier] = None,
                 n_neighbours: int = 2, n_jobs=None, random_state=None,
                 unknown_label: Union[str, int] = -1, enable_nearest_neighbour=True):

        self.enable_nearest_neighbour = enable_nearest_neighbour
        ...
        # Mazen: added condition
        if self.enable_nearest_neighbour:
    ...
    def _predict(self, X, n_neighbors: Optional[int] = None):
        ...
        # Mazen: added condition
        # Mazen: we don't need the leaves anymore, we just predict directly.
        if self.enable_nearest_neighbour:
    ...
    def predict(self, X, n_neighbors: Optional[int] = None):
        ...
        if self.enable_nearest_neighbour:
            ...
            return self._predict(X, n_neighbors)
